...
const determineMetric = (target: number, average: number): Metric => {
  ...
  if (average >= target) rating = 3, ratingDescription = 'Keep up the good work!';
  else if (average/target >= 0.75) rating = 2, ratingDescription = 'You are almost there!';
...
const parseArgs = (args: Array<string>): ExerciseArgs => {
  ...
  if (dailyHours.some(hour => isNaN(hour))) throw Error('Hours of work out must be numbers');
  return {
    target,
    dailyHours
  }
...
const vetExercises = (target: number, dailyHours: Array<number>): Result => {
  ...
  const average = dailyHours.reduce((a: number, b: number) => a + b)/periodLength;
